import { AlertTriangle, CalendarClock, Timer, Users } from 'lucide-react';
...
export default function Escala() {
  ...
  async function carregarBase() {
    ...
    setErro(null);
    try {
      const [ev, eq] = await Promise.all([listarEventos(), listarEquipe()]);
    ...
    <>
      ...
      <Conteudo>
        <MetricGrid>
          ...
          <MetricCard Icone={CalendarClock} rotulo="Confirmados (todos os eventos)" valor={String(confirmadosTotal)} legenda={`de ${escalas.length} convocados`} />
          <MetricCard Icone={Timer} rotulo="Prontos (traje+EPI)" valor={String(trajesOkTotal)} legenda={`de ${escalas.length} convocados`} />
          <MetricCard Icone={AlertTriangle} rotulo="Eventos com equipe faltando" valor={String(eventosComFalta)} legenda={`de ${eventos.length} eventos`} />
        </MetricGrid>
        ...
        {carregando ? (
          ...
          <div className="flex flex-col gap-4">
            {eventos.map((evento) => {
              ...
              return (
                <Panel key={evento.id} id={`evento-${evento.id}`} className={evento.id === eventoDoLink ? 'ring-1 ring-accent' : undefined}>
                        ...
                        <div key={esc.id} className="rounded-sm border border-line bg-input p-3 text-sm">
                          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <select value={esc.status} onChange={(e) => aoMudarStatus(esc.id, e.target.value as StatusEscala)} className="rounded-sm border border-line bg-panel px-2 py-1 text-[11.5px] text-text outline-none focus:border-accent">
                          ...
                          <div className="mb-2 flex flex-wrap items-center gap-4">
                            <label className="flex items-center gap-1.5 text-[12.5px] text-text-dim">
                              <input type="checkbox" checked={esc.traje_ok} onChange={(e) => aoMudarChecklist(esc.id, 'traje_ok', e.target.checked)} className="h-3.5 w-3.5 accent-accent" />
                              Traje OK
                            </label>
                            <label className="flex items-center gap-1.5 text-[12.5px] text-text-dim">
                              <input type="checkbox" checked={esc.epi_ok} onChange={(e) => aoMudarChecklist(esc.id, 'epi_ok', e.target.checked)} className="h-3.5 w-3.5 accent-accent" />
                              EPI OK
                            </label>
