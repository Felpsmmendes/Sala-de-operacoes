...
export function ModalCompra({ item, onFechar, onConfirmar }: { item: ItemEstoque; onFechar: () => void; onConfirmar: (quantidade: number, valorTotal: number) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onFechar}>
      <div className="w-full max-w-sm rounded-lg border border-line bg-panel p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="mb-4 block">
          ...
          <input type="number" min={0} step="0.01" value={valorTotal} onChange={(e) => setValorTotal(e.target.value)} placeholder="Ex: 450" className="w-full rounded-sm border border-line bg-input px-3 py-2.5 text-sm text-text outline-none focus:border-accent" />
        </label>

        <button
          type="button"
          disabled={!quantidade || Number(quantidade) <= 0}
          onClick={() => onConfirmar(Number(quantidade), Number(valorTotal) || 0)}
