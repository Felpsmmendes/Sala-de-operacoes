  ...
  return (
    <form onSubmit={aoSubmeter} className="grid grid-cols-1 gap-4 sm:grid-cols-4">
      <div className="sm:col-span-2">
        <Input rotulo="Nome da região" categoria="operacao" required value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex: Zona Sul" />
      </div>
      <div className="sm:col-span-2">
        <Input rotulo="Km aproximado (só ida — o cálculo dobra pra ida+volta)" categoria="operacao" type="number" min={0} step="0.1" required value={km} onChange={(e) => setKm(e.target.value)} placeholder="Ex: 15" />
      </div>
